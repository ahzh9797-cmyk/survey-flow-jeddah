...
function SharePreviewModal({ preview, survey, onClose }) {
 ...
 <div style={{background:R.dangerBg,border:`1px solid ${R.danger}30`,borderRadius:10,padding:"10px 14px",fontSize:12,color:R.danger,marginBottom:14}}>هذا الرابط منتهي الصلاحية</div>
 )}
 ...
 <span style={{fontSize:11,color:R.s400}}>· {preview.view_count||0} مشاهدة</span>
 </div>
 </div>
...
function ReviewDetailModal({ preview, survey, user, onClose }) {
 ...
 <p style={{margin:"0 0 2px",fontSize:11,fontWeight:700,color:R.s700}}>{r.reviewer_name} ↩</p>
 ...
 {audit.map((a,i)=>{
 const icons = { preview_created:"", preview_opened:"", comment_added:"", comment_resolved:"", approval_granted:"", approval_rejected:"", review_completed:"" };
 return (
...
export default function ReviewCenter({ surveys, user }) {
 ...
 <p style={{margin:"0 0 4px",fontSize:12,fontWeight:700,color:R.e700}}>وضع المراجعة الآمن</p>
...
export function ReviewPreviewPage({ token, surveys }) {
 ...
 <p style={{margin:0,fontSize:12,color:R.warn,fontWeight:700}}>هذا وضع المعاينة — لن تُحفظ أي بيانات</p>
 </div>
