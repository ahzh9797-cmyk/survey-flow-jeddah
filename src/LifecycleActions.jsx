...
function ConfirmDialog({ action, survey, onConfirm, onCancel }) {
 const configs = {
  ...
  pause:   { title: "إيقاف مؤقت؟",     desc: "لن يتمكن أحد من الإجابة أثناء الإيقاف.",        btn: "إيقاف مؤقت", variant: "secondary" },
  resume:  { title: "استئناف الاستبيان؟",  desc: "سيعود الاستبيان للعمل ويقبل إجابات جديدة.",       btn: " استئناف",   variant: "primary" },
...
export default function LifecycleActions({ survey, user, isAdmin, onRefresh, compact = false }) {
 ...
 const ACTION_CONFIG = {
  ...
  pause:   { label: "إيقاف مؤقت", variant: "secondary" },
  resume:  { label: "استئناف",  variant: "primary"  },
 ...
 return (
  <>
   ...
   {error && (
    <p style={{ color: C.danger, fontSize: 12, margin: "6px 0 0" }}> {error}</p>
   )}
