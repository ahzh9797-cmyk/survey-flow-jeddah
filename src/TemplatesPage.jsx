        ...
        <div style={{ padding:"12px 18px 0" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:16 }}>
            {[
              ...
              { i:"⏱️", l:"الوقت",    v:`${minutes} د` },
              { i:"🗂️", l:"الفئة",    v: template.template_categories?.name || "—" },
            ].map((x, i) => (
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:200,
      ...
      <div style={{ width:"100%", maxWidth:560, margin:"0 auto", background:TT.white, borderRadius:"24px 24px 0 0",
        ...
        <div style={{ padding:"8px 18px 0" }}>
          ...
          {error && <div style={{ background:TT.dangerBg, border:"1px solid #FECACA", borderRadius:10, padding:"10px 14px", fontSize:13, color:TT.danger, marginBottom:12, display:"flex", gap:8 }}><span>⚠️</span>{error}</div>}
      ...
      <div style={{ padding:"14px 16px", flex:1, display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ display:"flex", gap:12, marginBottom:12, flexWrap:"wrap" }}>
          ...
          {template.template_categories?.name && (
            <span style={{ fontSize:11, color:TT.s400, display:"flex", alignItems:"center", gap:3 }}>🗂️ {template.template_categories.name}</span>
          )}
        </div>
