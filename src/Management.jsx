        ...
        if (s.survey_type === "school" && s.approval_status === "approved") {
          ...
          while (true) {
            const { data } = await supabase.from("survey_schools")
              .select("id,name,principal,phone,stage").range(from, from+999);
        ...
        <>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:12, marginBottom:20 }}>
            {[
              ...
              <div key={i} onClick={()=>x.nav && onNavigate && onNavigate(x.nav)}
                className={x.nav ? "card-hover" : undefined}
                style={{ background:C.white, borderRadius:16, padding:16, border:`1px solid ${C.border}`,
                  borderTop:`3px solid ${x.c}`, boxShadow:"0 2px 8px rgba(0,0,0,0.06)",
                  cursor:x.nav?"pointer":"default", textAlign:"center" }}>
          ...
          {activeSurveys.length === 0 ? (
            ...
            const expiresTomorrow = s.expires_at &&
              (new Date(s.expires_at) - new Date()) < 24*60*60*1000 && new Date(s.expires_at) > new Date();
            return (
...
function SchoolForm({ initial, onSaved, onCancel, user }) {
  ...
  async function save() {
    ...
    logAction({ user, action: isEdit ? "update" : "create", table: "survey_schools", recordId: payload.id, recordLabel: payload.name });
    onSaved();
  }
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:50, display:"flex", alignItems:"flex-end" }}>
      <div style={{ background:C.bg, width:"100%", maxHeight:"92vh", overflowY:"auto", borderRadius:"18px 18px 0 0", padding:18, direction:"rtl" }}>
        ...
        <input value={form.id} onChange={e=>set("id", e.target.value)} disabled={isEdit}
          style={{ ...inputStyle, direction:"ltr", textAlign:"center", fontWeight:700, background: isEdit ? "#f0f0f0" : "#fff" }}/>
  ...
  function handleFile(e) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (parsed.length === 0) setError("لم يتم العثور على بيانات صالحة في الملف.");
        setRows(parsed);
      ...
      <div style={{ background:C.bg, width:"100%", maxHeight:"90vh", overflowY:"auto", borderRadius:"18px 18px 0 0", padding:18, direction:"rtl" }}>
          ...
          <>
            <Card style={{ marginBottom:14, background:C.primaryBg }}>
              <p style={{ margin:0, fontSize:12, color:C.text, lineHeight:1.8 }}>
                الأعمدة المطلوبة: <strong>id, name</strong> (إلزامية)، ويمكن أيضاً: principal, phone, email, stage, supervisor, status
              </p>
            </Card>
            ...
            {!parsing && rows.length > 0 && (
              <Card style={{ marginBottom:14 }}>
                <p style={{ margin:"0 0 8px", fontSize:13, fontWeight:700, color:C.success }}>✅ تم العثور على {rows.length} مدرسة</p>
                <div style={{ maxHeight:160, overflowY:"auto" }}>
                  {rows.slice(0, 5).map((r, i) => (
                    <div key={i} style={{ fontSize:11, color:C.muted, padding:"4px 0", borderBottom:`1px solid ${C.border}` }}>{r.id} — {r.name}</div>
                  ))}
        ...
        {result && (
          <div style={{ textAlign:"center", padding:20 }}>
            ...
            <div style={{ marginTop:18 }}><Btn full onClick={onDone}>تم</Btn></div>
          </div>
        )}
      </div>
    </div>
...
function DeleteConfirm({ school, onConfirm, onCancel, user }) {
  ...
  async function doDelete() {
    ...
    setDeleting(false);
    if (!err) logAction({ user, action: "delete", table: "survey_schools", recordId: school.id, recordLabel: school.name });
    onConfirm();
  }
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:50, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:"#fff", borderRadius:16, padding:22, maxWidth:340, width:"100%", direction:"rtl" }}>
        ...
        <p style={{ textAlign:"center", fontSize:13, color:C.muted, margin:"0 0 18px" }}>
          هل أنت متأكد من حذف مدرسة<br/><strong style={{ color:C.text }}>{school.name}</strong>؟<br/>لا يمكن التراجع.
        </p>
...
function SchoolsManagementPage({ isAdmin, user }) {
  ...
  const load = useCallback(async () => {
    ...
    const BATCH = 1000;
    while (true) {
      const { data, error } = await supabase.from("survey_schools").select("*").order("name").range(from, from + BATCH - 1);
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(s => s.id.toLowerCase().includes(q) || (s.name||"").toLowerCase().includes(q) || (s.principal||"").toLowerCase().includes(q));
    }
    return list;
...
function UsersManagementPage({ currentUser }) {
  ...
  async function approve(userId, displayName) {
    ...
    logAction({ user: currentUser, action: "update", table: "user_roles", recordId: userId, recordLabel: `قبول طلب تسجيل: ${displayName || "مستخدم"}` });
  ...
  async function reject(userId, displayName) {
    ...
    logAction({ user: currentUser, action: "update", table: "user_roles", recordId: userId, recordLabel: `رفض طلب تسجيل: ${displayName || "مستخدم"}` });
    load();
  }
        ...
        <>
          {pending.length > 0 && (
            <div style={{ marginBottom:20 }}>
              <p style={{ margin:"0 0 10px", fontSize:13, fontWeight:700, color:C.warn }}>🔔 طلبات تسجيل بانتظار الموافقة ({pending.length})</p>
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {pending.map(r => (
                  <Card key={r.user_id} style={{ padding:14, background:C.warnBg, border:`1px solid ${C.warn}40` }}>
                    ...
                    <p style={{ margin:"0 0 10px", fontSize:11, color:C.muted }}>طلب التسجيل {new Date(r.created_at).toLocaleDateString("ar-SA")}</p>
                    <div style={{ display:"flex", gap:8 }}>
          ...
          {others.length === 0 ? (
            <Card style={{ textAlign:"center", padding:24 }}><p style={{ margin:0, color:C.muted, fontSize:13 }}>لا يوجد مستخدمون آخرون بعد</p></Card>
          ) : (
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {others.map(r => (
                    ...
                    <div style={{ display:"flex", gap:8 }}>
                      <button onClick={()=>setRole(r.user_id, "admin", r.display_name)} disabled={r.role==="admin"}
                        style={{ flex:1, padding:"7px 0", borderRadius:8, fontSize:11, fontFamily:"inherit",
                          ...
                          cursor: r.role==="admin" ? "default" : "pointer", fontWeight:700 }}>👑 مدير عام</button>
                      <button onClick={()=>setRole(r.user_id, "viewer", r.display_name)} disabled={r.role==="viewer"}
                        style={{ flex:1, padding:"7px 0", borderRadius:8, fontSize:11, fontFamily:"inherit",
                          ...
                          cursor: r.role==="viewer" ? "default" : "pointer", fontWeight:700 }}>👁️ مشرف (عرض فقط)</button>
                    </div>
                  )}
                  {r.status === "rejected" && (
...
const TABLE_LABELS = {
  survey_schools: "المدارس", surveys: "الاستبيانات",
  survey_questions: "أسئلة الاستبيان", user_roles: "صلاحيات المستخدمين",
};
...
function SupervisorsManagementPage({ user }) {
  ...
  async function parseFile(file) {
    ...
    if (ext === "pdf") {
      ...
      for (const line of lines) {
        ...
        if (idMatch) {
          let name = line.replace(idMatch[0], "").replace(phoneMatch?.[0]||"", "")
            .replace(/\d+/g, "").replace(/[#\-_|،,]/g, " ").trim().replace(/\s+/g, " ");
          if (name.length > 2) rows.push({ name, national_id: idMatch[0], phone: phoneMatch?.[0] || "", email: "", status: "مُسندة" });
        }
      }
      ...
      const mapped = rows.map(r => ({
        ...
        phone: String(r["الجوال"]||r["phone"]||r["Phone"]||"").trim(),
  ...
  return (
    <div style={{ padding:16, direction:"rtl" }}>
            ...
            {paged.length === 0 ? (
              ...
              <div key={s.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 14px",
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <p style={{ margin:"2px 0 0", fontSize:11, color:C.muted }}>هوية: {s.national_id} {s.phone && `· 📱 ${s.phone}`}</p>
                </div>
                <Tag color={s.status==="مُسندة"?C.success:C.muted}>{s.status}</Tag>
                <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                  <button onClick={()=>setForm(s)} style={{ background:C.primaryBg, border:"none", borderRadius:8, padding:"5px 8px", fontSize:11, color:C.primary, cursor:"pointer", fontFamily:"inherit" }}>✏️</button>
                  <button onClick={()=>setDelTarget(s)} style={{ background:"#fdf0ee", border:"none", borderRadius:8, padding:"5px 8px", fontSize:11, color:C.danger, cursor:"pointer", fontFamily:"inherit" }}>🗑️</button>
                </div>
              </div>
            ))}
          </Card>
      ...
      {form !== null && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:100, display:"flex", alignItems:"flex-end", direction:"rtl" }}>
          <div style={{ width:"100%", background:C.white, borderRadius:"20px 20px 0 0", padding:20, maxHeight:"80vh", overflowY:"auto" }}>
            ...
            {[["الاسم","name","اسم المشرف"],["رقم الهوية","national_id","10 أرقام"],["رقم الجوال","phone","05xxxxxxxx"],["البريد الإلكتروني","email",""]].map(([l,k,ph]) => (
              <div key={k}>
                <label style={{ display:"block", fontSize:12, fontWeight:700, color:C.text, marginBottom:4 }}>{l}</label>
                <input value={form[k]||""} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} placeholder={ph} style={inputStyle}/>
              </div>
            ))}
            <label style={{ display:"block", fontSize:12, fontWeight:700, color:C.text, marginBottom:4 }}>الحالة</label>
            <select value={form.status||"مُسندة"} onChange={e=>setForm(p=>({...p,status:e.target.value}))} style={{ ...inputStyle, background:C.white }}>
            ...
            {!previewRows ? (
              <>
                <Card style={{ marginBottom:12, background:C.primaryBg }}>
                  <p style={{ margin:"0 0 6px", fontSize:12, color:C.text, lineHeight:1.8 }}><strong>Excel/CSV:</strong> أعمدة: الاسم، رقم الهوية (إلزامي)، الجوال، البريد، الحالة</p>
                  <p style={{ margin:0, fontSize:12, color:C.warn, lineHeight:1.8 }}><strong>PDF:</strong> يتم استخراج البيانات تلقائياً لكن قد يحتاج مراجعة</p>
                </Card>
                <ErrorBanner message={error}/>
                <input type="file" accept=".xlsx,.xls,.csv,.pdf" onChange={e=>{ if(e.target.files?.[0]) parseFile(e.target.files[0]); }} style={{ width:"100%", fontSize:13, marginBottom:12 }}/>
                <Btn full variant="secondary" onClick={()=>{setCsvOpen(false);setError("");}}>إلغاء</Btn>
              </>
            ) : (
              <>
                <div style={{ background:C.successBg, border:`1px solid ${C.success}40`, borderRadius:10, padding:"10px 14px", fontSize:12, color:C.success, marginBottom:12 }}>
                ...
                <div style={{ overflowX:"auto", marginBottom:14 }}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                    <thead><tr style={{ background:C.primaryBg }}>
                      {["الاسم","رقم الهوية","الجوال"].map(h => <th key={h} style={{ padding:"8px 10px", textAlign:"right", color:C.primary, borderBottom:`1px solid ${C.border}` }}>{h}</th>)}
                    </tr></thead>
                    <tbody>
                  ...
                  {previewRows.length > 20 && <p style={{ textAlign:"center", fontSize:11, color:C.muted, marginTop:6 }}>... و{previewRows.length-20} سجل آخر</p>}
                </div>
                <div style={{ display:"flex", gap:8 }}>
