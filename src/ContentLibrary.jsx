...
const Q_TYPES = [
 {v:"text",l:"نص قصير",icon:""},{v:"textarea",l:"نص طويل",icon:""},
 {v:"number",l:"رقم",icon:""},{v:"select",l:"اختيار",icon:""},
...
function VersionHistoryModal({ type, itemId, onRestore, onClose }) {
 ...
 {i > 0 && (
 <LBtn sm variant="secondary" onClick={()=>onRestore(v)}>استعادة هذه النسخة</LBtn>
 )}
 </div>
 ))}
 </Modal>
...
function QuestionLibraryTab({ categories, user, onInsert }) {
 ...
 <LBtn sm variant="secondary" onClick={()=>setFormTarget(item)}>تعديل</LBtn>
 ...
 <LBtn sm variant="danger" onClick={()=>handleDelete(item.id)}></LBtn>
 </div>
 </div>
 </div>
 ))}
...
function ConditionsTab({ categories, user }) {
 ...
 <LBtn sm variant="secondary" onClick={()=>openForm(item)}>تعديل</LBtn>
 <LBtn sm variant="danger" onClick={()=>handleDelete(item.id)}></LBtn>
 </div>
 </div>
 </div>
